...
namespace SumoTrafficGenerator
{
    public partial class SumoGenTraffic_Form : Form
    {
        ...
        private void generateTraffic_button_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (density_trackBar.Value == 0)
                    ...
                    periodicity = (1 / density_trackBar.Value).ToString().Replace(",", ".");
        ...
        {
            string sumoHomeDirectory, destinationDirectory;
            ...
            try
            {
                ...
                sumoHomeDirectory = Environment.GetEnvironmentVariable("SUMO_HOME");
                ...
                System.IO.File.Copy(sumoHomeDirectory + "/tools/trafficGenerator/files/start.bat",
                    destinationDirectory + "/start.bat", true);

                System.IO.File.Copy(sumoHomeDirectory + "/tools/trafficGenerator/files/map.local.sumocfg",
                    destinationDirectory + "/map.local.sumocfg", true);

                System.IO.File.Copy(sumoHomeDirectory + "/tools/trafficGenerator/files/map.gui.sumocfg",
                    destinationDirectory + "/map.gui.sumocfg", true);

                // Run the script gensumofiles.bat in location given
                sumoBatchProcess = System.Diagnostics.Process.Start(sumoHomeDirectory + "/tools/trafficGenerator/files/gensumofiles.bat ", minLat + " " + minLon + " " + maxLat + " " + maxLon + " " + periodicity + " " + destinationDirectory);
